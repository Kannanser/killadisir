...
re codded by afnan plk and kannanser said
*/
const fs = require('fs')
...
Asena.addCommand({on: 'text', fromMe: false }, (async (message, match) => {
    if(Config.BGMFILTER){
        ...
        if (!!message.mention && message.mention[0] == '919633687665@s.whatsapp.net') {
...
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
       await message.client.sendMessage(message.jid, fs.readFileSync('./uploads2/' + a + '.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio,duration: 9999999, contextInfo: { forwardingScore: 1000, isForwarded: true }, quoted: message.data, ptt: true})
}
});
    }
